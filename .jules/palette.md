...
## 2026-08-01 - Command-specific Hero alignment and Context-aware Empty States
**Learning:** Commands sharing the same reporting/execution pipeline (like `search` and `report`) can easily inherit incorrect heroes or generic empty-state guidance. By conditionally checking pipeline flags (such as `all_events` or `only_project`), the CLI can render tailored command-specific heroes and highly context-aware "Next:" hints that guide the user to adjust the relevant flags.
**Action:** Implemented a dedicated `"search"` hero in `outputs/cli_heroes.py` and supported it in `outputs/terminal.py` based on `args.all_events`. Refined `core/report_cli.py` to print a tailored `Next: gittan search --today --noise-profile lenient` troubleshooting cue for search empty states, and added guidance for project-filtered empty states to run without the project filter. Added corresponding tests in `tests/test_cli_report_empty_state.py`.
