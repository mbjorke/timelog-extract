...
| `docs/specs/local-evidence-shadow-log.md`              | `partial`      | Slice 1 (record contract + volume spike, GH-151/#153), item 3 (durable opt-in `--shadow-log on` capture to a local evidence store directory, append-only JSONL + per-month hash chain), and item 5 (`gittan evidence` health surface), and item 4 (`--shadow-replay on` restores stored evidence for closed windows after source rotation) are built. Remaining: retention/compaction, redaction defaults, export/erase (all `later`). Backlog: `docs/task-prompts/local-evidence-shadow-log-slice1-task.md`. Engine decided JSONL-first from measured volume; pg_duckdb ruled out. |
