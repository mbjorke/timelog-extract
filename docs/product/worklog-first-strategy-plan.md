...
- Scenario A in `docs/runbooks/manual-test-matrix-0-2-x.md` can look "empty" even when sources are accessible, because classification/filtering depends on fallback profile keywords.
...
- Document behavior in `README.md` and `docs/sources/sources-and-flags.md`.
