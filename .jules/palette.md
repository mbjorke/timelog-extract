...
<<<<<<< HEAD
## 2026-07-25 - Standardizing `projects-audit`, `projects-trim`, and `projects-anchor` CLI Output and Error/Plan Warnings
**Learning:** Legacy diagnostic and configuration modification commands (like `projects-audit`, `projects-trim`, and `projects-anchor`) often fall back to raw or ad-hoc terminal colors (`[red]`, `[yellow]`, `[green]`) and high-contrast unstyled tables. Standardizing them with Gittan CLI UX tokens (`CLR_GREEN`, `CLR_VALUE_ORANGE`, `STYLE_MUTED`, `STYLE_DIM`, `OK_ICON`, `WARN_ICON`, `FAIL_ICON`) and `box.ROUNDED` tables alongside actionable `Next:` guidance preserves visual cohesion, scannability, and seamless interactive flow.
**Action:** Aligned `core/cli_projects_audit.py` with standard Gittan CLI UX patterns. Standardized tables to `box.ROUNDED` with `STYLE_BORDER`, and replaced ad-hoc formatting strings with semantic tokens and icon redundancy. Added structured, actionable `Next:` guidelines for error and plan-generation outcomes, and refactored validation inputs to keep the file under the strict 500-line limit.
=======
...
>>>>>>> bdcc1ca (palette: enhance review decidability and label provenance)
