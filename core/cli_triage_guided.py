...
def _resolve_recent_range(date_from: Optional[str], date_to: Optional[str]) -> tuple[Optional[str], Optional[str]]:
    if date_from or date_to:
        return date_from, date_to
    end_d = date.today()
    start_d = end_d - timedelta(days=DEFAULT_GUIDED_RECENT_DAYS - 1)
...
def _render_uncategorized_fallback(*, report, date_from: Optional[str], date_to: Optional[str], max_days: int) -> str | None:
    ...
    return (
        ...
        "Next: run `gittan triage --json` and prepare explicit decisions for `gittan triage-apply` "
...
def triage_guided(
    ...
    console = Console()
    date_from, date_to = _resolve_recent_range(date_from, date_to)
    ...
    if not days:
        ...
        if fallback:
            ...
            if write_decisions:
                written_path = _write_decisions_file(write_decisions, [])
                console.print(
                    "[green]Scaffold decisions file written:[/green] "
                    f"{written_path} (no auto-derived decisions in fallback mode)"
                )
