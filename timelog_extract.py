#!/usr/bin/env python3
"""timelog_extract.py — compatibility facade + report orchestration."""
...
from core.config import (
    default_worklog_path as core_default_worklog_path,
    load_profiles,
    normalize_profile,
    resolve_worklog_path as core_resolve_worklog_path,
)
...
def resolve_worklog_path(cli_worklog, config_path, workspace_worklog):
    return core_resolve_worklog_path(cli_worklog, config_path, workspace_worklog, SCRIPT_DIR)
...
def dedupe_events(events):
    return core_dedupe_events(events, event_key)
...
def run_timelog_report(config_path, date_from, date_to, options):
    ...
    runtime_collectors = RuntimeCollectors(
        ...
        classify_project_fn=classify_project,
        make_event_fn=make_event,
