...
| Classified candidates | Anchor unmapped **stable** signals (repo/dir → match_terms, hosts → tracked_urls); branch/label stay inventory | `gittan projects-audit --write-anchor-plan` / `projects-anchor` | Default min_hits≥20; ephemeral kinds need `--include-ephemeral-kinds`. Prefer `gittan map` to attach repo/dir to an existing line. |
