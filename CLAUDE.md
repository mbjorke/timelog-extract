...
## Task types and scope discipline

Understanding what type of task you have been given is more important than starting work quickly. Read the brief, identify the task type, and deliver exactly that — nothing more, nothing less.

### Review tasks

A **review task** means: **read, analyse, and write a document**. It does not mean fix code.

When a handover, issue, or prompt asks you to review the codebase, a PR, or a strategy document:

1. **Deliver the review document first.** That is the output. Commit it as its own standalone deliverable on the feature branch.
2. **List findings as findings.** If you spot a bug or security issue while reading, note it clearly in the review document with a recommended fix. Do not implement the fix during the review pass.
3. **Ask before acting.** If a finding is severe enough that you believe a fix should accompany the review, say so explicitly and wait for confirmation before making code changes.

**Why this matters:** a review that also silently patches code conflates two different deliverables, makes the PR harder to evaluate, and may be rejected or ignored even if the fix itself was correct. The review document is the finish line; code changes are a separate race.

**What went wrong in practice (April 2026):** a review task on branch `claude/gittan-launch-review-4a5DD` produced the correct review document but bundled an unasked-for security fix into the same commit. Marcus cherry-picked the fix into `main` manually (PR #14), but the review branch was never treated as a proper PR because its scope was muddled. The fix landed; the review did not get a proper evaluation cycle.

### Implementation tasks

An implementation task means: make the requested change, run tests, commit, push. The deliverable is working code on the branch, not prose.

### Hybrid tasks (review + fix)

If the brief explicitly asks for both analysis and code changes in the same task, scope them into separate commits in this order:

1. Commit 1: the review/analysis document.
2. Commit 2+: the code changes, clearly referencing the findings from commit 1.

This keeps each deliverable reviewable on its own merits.
