...
Current sources (in `core/sources.py`): Claude Code CLI, Claude Desktop, Claude.ai (web), Gemini (web), Cursor, Cursor checkpoints, Antigravity (Google Antigravity IDE), Devin Desktop (Cognition; formerly Windsurf — also reads legacy Windsurf app-support dirs), VS Code (stock `Code` / `Code - Insiders`), Codex IDE, Gemini CLI, TIMELOG.md, Apple Mail, Chrome, WordPress (derived from Chrome wp-admin / "— WordPress" titles), Lovable (web) (derived from Chrome lovable.dev / *.lovableproject.com), Lovable (desktop) (Electron app under Application Support/lovable-desktop), GitHub, Calendar (local macOS, opt-in via `--calendar-source on`).
