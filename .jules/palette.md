...
## 2026-07-26 - Standardizing Interactive UX Escape Hatch and Cancellation
**Learning:** Interactive commands that prompt for user choices or edits must reliably support a clean cancellation/abort path. If a user interrupts (Ctrl+C / Escape / selects 'Cancel' or empty inputs), the CLI must immediately terminate the session by raising `typer.Exit(code=130)` and display a clean cancellation message styled using `CLR_VALUE_ORANGE` instead of proceeding with empty inputs or spamming normal 'Next step' guidelines.
**Action:** Updated `core/cli_projects.py`, `core/cli_url_mapping.py`, `core/cli_review_gaps.py`, `core/cli_review_create_project.py`, and `core/cli_review_uncategorized.py` to capture `None` outputs from all interactive `questionary` prompts and raise `typer.Exit(code=130)` with a standardized orange cancellation message. Added `tests/test_cli_projects_cancellation.py` to assert this escape hatch behaviour.
**Correction (review follow-up):** "capture `None`" is only half the contract, and treating it as the whole thing introduced three defects. `questionary.confirm()` also returns `False` for a deliberate "no" — routing that to 130 reported a considered decline as an interrupt. `.ask() or ""` collapses `None` into an empty string, so the cancellation never reaches the guard at all. And a helper that returns `None` for both cancellation *and* recoverable validation failures (`prompt_new_project_fields`) leaves the caller unable to tell them apart, so it continues past a quit. Distinguish the three states — interrupted, declined, invalid — before mapping any of them to an exit code. Cancellation messages must also match what is already on disk: loops that save per iteration cannot claim "before writing config" once one iteration has been applied.
