...
Runs ``run_timelog_report`` twice (default v1 classifier vs injected work-unit
...
def main() -> int:
    ...
    if not args.skip_baseline:
        baseline_opts = TimelogRunOptions(
        ...
        baseline = run_timelog_report(
            args.projects_config, date_from, date_to, baseline_opts
        )
        baseline_unc = float(hours_by_project(baseline).get("Uncategorized", 0.0) or 0.0)

    spike_opts = TimelogRunOptions(
        projects_config=args.projects_config,
        date_from=date_from,
        date_to=date_to,
        include_uncategorized=True,
    ...
    spike_report = run_timelog_report(
        args.projects_config, date_from, date_to, spike_opts
    )
    verdict = evaluate_spike(
        spike_report,
        acceptance,
        profiles=getattr(spike_report, "profiles", None) or [],
    ...
    payload = {
        ...
        "after_hours_by_line": hours_by_project(spike_report),
    }
