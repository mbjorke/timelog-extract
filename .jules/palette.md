...
## 2026-08-04 - Standardizing `calendar-suggest` CLI Output and Handling ValueError Config Fallbacks
**Learning:** Commands like `calendar-suggest` that do not strictly require an existing project configuration should handle missing config files or empty profiles gracefully by catching `ValueError` to prevent tracebacks. Additionally, formatting suggestions with a rounded table using standard theme tokens and syntax highlighting the config block with the ansi theme enhances visual scannability and actionability.
**Action:** Wrapped `load_profiles` in `core/cli_calendar_suggest.py` in a try...except ValueError fallback block. Redesigned output using standard Rich components (rounded table with `box.ROUNDED`, `STYLE_BORDER`, and `STYLE_LABEL` headers) and highlighted proposed config block with `Syntax(..., "json", theme="ansi")`.

## 2026-08-05 - Standardizing `projects-audit` Tables with Gittan CLI UX guidelines
**Learning:** Dense audit summary commands (like `projects-audit`) can drift into using ad-hoc non-semantic styling, missing rounded borders, and custom header styles, causing poor visual scannability and breaking the cohesive terminal aesthetic. Utilizing the official theme tokens (`STYLE_BORDER`, `STYLE_LABEL`) and standard `box.ROUNDED` border styles ensures all CLI commands speak the same visual language.
**Action:** Replaced ad-hoc bold header styles and missing table box/border definitions with `box.ROUNDED`, `STYLE_BORDER`, and `STYLE_LABEL` in the table representations within `core/cli_projects_audit.py`.
