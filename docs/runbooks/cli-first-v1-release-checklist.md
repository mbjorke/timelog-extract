...
- Confirm v1 scope is CLI-first (engine + reports + PDF + JSON/HTML exports).
- Keep extension labeled as scaffold/beta (not required for core usage).
...
- `scripts/run_engine_report.py --today --pdf` works on clean clone + fresh env.
- `timelog_extract.py --today --format json` outputs valid truth payload (`schema`, `version` present).
- PDF generation works for both non-empty and empty ranges.
...
- `core.engine_api.run_report_payload(...)` is treated as stable contract.
- Truth payload schema/version is documented in code/docs.
- Boundary tests assert `schema`, `version`, and totals block.
- Any payload key changes are release-noted before tagging.

## 5) Privacy + Trust Copy

- "Local-only processing" statement appears in README and release notes.
- Privacy-sensitive sources are clearly called out (Mail/Chrome/Screen Time).
- No cloud upload path implied in v1 docs or examples.
- Consent/toggle UX is marked as extension-beta work, not v1 blocker.

## 6) Release Artifacts

- `CHANGELOG.md` has a clear v1 section (why this release matters).
- Tag annotation draft is prepared (`v1.0.0` notes).
- Example commands in README are copy-paste verified.
- One sample output path exists in docs (`output/latest-payload.json`, PDF path example).
...
- `python3 timelog_extract.py --today --source-summary` output is readable and non-erroring.

Use `docs/runbooks/rc-test-script-cli-first.md` for second-tester execution and report collection.
