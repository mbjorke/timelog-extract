...
| Privacy/consent regressions | ActivityWatch's core positioning: local data ownership and optional collection scope. | **Adopt** | Keep first-run consent and per-source toggles as non-negotiable (`docs/security/privacy-security.md`). Treat new sources as opt-in by default until explicit product decision says otherwise. |
...
| Scope creep into "everything tracker" | gtimelog remains intentionally narrow; ActivityWatch's breadth increases operational complexity. | **Adopt** | Keep v1 source scope constrained to `docs/product/v1-scope.md`. Gate any additional source behind explicit ROI and accuracy impact, not "possible to collect". |
...
3. In report output, make source-level evidence easy to inspect when attribution is weak (aligns with `docs/product/accuracy-plan.md` Iteration 2).
