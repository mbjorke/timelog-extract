...
| `docs/specs/working-directory-anchor-signal.md`        | `built`        | Claude Code CLI events preserve `context_dir` (cwd leaf); `projects-audit` emits `top_dirs` + `--write-anchor-plan`; `projects-anchor` applies match_term additions (dry-run + backup). Tests: `tests/test_collectors_claude_code_context_dir.py`, `tests/test_projects_audit.py`. Follow-up: extend `context_dir` to Cursor/Gemini collectors. |
