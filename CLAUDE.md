...
```bash
python3 -m venv .venv
.venv/bin/python -m pip install -e .   # editable install from repo root
```
...
Current sources (in `core/sources.py`): Claude Code CLI, Claude Desktop, Claude.ai (web), Gemini (web), Cursor, Cursor checkpoints, Antigravity (Google Antigravity IDE), Devin Desktop (Cognition; formerly Windsurf — also reads legacy Windsurf app-support dirs), Codex IDE, Gemini CLI, TIMELOG.md, Apple Mail, Chrome, WordPress (derived from Chrome wp-admin / "— WordPress" titles), Lovable (web) (derived from Chrome lovable.dev / *.lovableproject.com), Lovable (desktop) (Electron app under Application Support/lovable-desktop), GitHub, Calendar (local macOS, opt-in via `--calendar-source on`). There is no stock VS Code (`Code` / `Code - Insiders`) collector.
