...
| `docs/specs/local-evidence-shadow-log.md`              | `partial`      | Slice 1 (record contract + volume spike, GH-151/#153) and item 3 (durable opt-in `--shadow-log on` capture to `~/.gittan/evidence/`, append-only JSONL + per-month hash chain) are built; replay for closed windows + health-monitor surface remain. Backlog: `docs/task-prompts/local-evidence-shadow-log-slice1-task.md`. Engine decided JSONL-first from measured volume; pg_duckdb ruled out. |
