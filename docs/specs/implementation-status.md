...
| `docs/specs/working-directory-anchor-signal.md`        | `built`        | Generalized to a multi-kind **activity anchor** model: events carry an `anchors` map — `dir` (Claude Code/Cursor/Windsurf/Antigravity/Gemini CLI), `branch` (Claude Code `gitBranch`), `label` (Codex `thread_name`). `projects-audit` emits a unified **`top_signals`** model (schema v2; folds the former `top_hosts` + `top_anchors`, each row tagged with `rule_type`: hosts → `tracked_urls`, anchors → `match_terms`) + `--write-anchor-plan`; `projects-anchor` applies both rule types (dry-run + backup). Auto-surfaced in `status` (one-line warning + interactive map, `--no-anchor-nudge`) and `report` (multi-line nudge). Tests: `test_collectors_claude_code_context_dir.py`, `test_projects_audit.py`, `test_report_nudges.py`, `test_anchor_nudge.py`. Follow-up: `label` coverage beyond Codex (Claude Code `~/.claude.json` history, web tab titles), React Ink overlay. |
