...
_CATEGORY_WORDS = (
    "security", "bug", "correctness", "reliability", "performance",
    "potential issue", "possible issue", "maintainability", "refactor",
    "nitpick", "typo", "documentation",
)
_SEVERITY_BADGES = (
...
def _pr_inline_findings(slug: str, number: int) -> list[dict]:
    ...
    raw = _run(["gh", "api", f"repos/{slug}/pulls/{number}/comments?per_page=100"])
    try:
...
def main() -> int:
    ...
    gh.add_argument("--deep", action="store_true",
