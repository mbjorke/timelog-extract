...
def _is_managed_hook(path: Path) -> bool:
    ...
    try:
        ...
        return False
    return "managed-by-gittan: global-timelog" in content
...
def _discover_git_repos() -> list[Path]:
    ...
    for root in roots:
        ...
        for git_dir in root.glob("**/.git"):
            ...
            if not repo.is_dir():
                continue
            if "/.cache/" in str(repo) or "/Library/" in str(repo):
                continue
...
def _configure_timelog_scope_and_name(console, *, yes: bool, dry_run: bool) -> None:
    ...
    if yes:
        ...
        timelog_name = (
            questionary.text(
                "Timelog file path inside each repo (relative path):",
                default="TIMELOG.md",
            ).ask()
            or "TIMELOG.md"
        ).strip()
        scope_mode = questionary.select(
            ...
            choices=[
                "All git repositories (recommended default)",
                "Only selected repositories (scan and choose)",
            ],
        ...
        if scope_mode.startswith("Only selected"):
            ...
            if repos:
                selected = questionary.checkbox(
                    ...
                    choices=[str(repo) for repo in repos],
                ).ask() or []
            else:
...
def _setup_global_timelog_impl(console, *, yes: bool, dry_run: bool) -> None:
    from rich.console import Console
    from rich.table import Table  # noqa: F401 - type import parity
    ...
    console.print(
        "This will configure global git hooks so each commit appends an entry to repo-local `TIMELOG.md`."
    )
    ...
    proceed = yes or questionary.confirm("Proceed with setup?", default=True).ask()
    if not proceed:
    ...
    try:
        ...
        if hook_path.exists():
            if _is_managed_hook(hook_path):
                ...
                console.print(
                    "[yellow]Existing hook is not managed by Gittan, so overwrite confirmation is required.[/yellow]"
                )
        ...
        if (not hook_path.exists() and not dry_run) or dry_run:
            pass
        else:
    ...
    if added_ignore:
        console.print("Added `TIMELOG.md` to global gitignore.")
    else:
        console.print("`TIMELOG.md` already present in global gitignore.")
    ...
    if not dry_run and hook_path.exists():
        ...
        timelog_name = (
            GITTAN_FILENAME_FILE.read_text(encoding="utf-8").splitlines()[0].strip()
...
def _ensure_minimal_projects_config(console, *, yes: bool, dry_run: bool) -> None:
    ...
    should_create = yes or questionary.confirm(
        f"Create minimal project config at {config_path}?",
        default=True,
    ).ask()
...
def _print_environment_status(console) -> None:
    ...
    if gittan_in_path:
        table.add_row("`gittan` command", "[green]OK[/green]", gittan_in_path)
    else:
        table.add_row("`gittan` command", "[yellow]MISSING[/yellow]", "Command not currently available in PATH")

    if scripts_in_path:
        table.add_row("Python scripts PATH", "[green]OK[/green]", scripts_path)
    else:
        table.add_row(
            "Python scripts PATH",
            "[yellow]ACTION[/yellow]",
            f"Add to PATH: {scripts_path}",
        )
    ...
    table.add_row(
        "GitHub token env",
        "[green]SET[/green]" if github_token else "[dim]optional[/dim]",
    ...
    if not scripts_in_path:
        ...
        console.print(
            f"[yellow]Tip:[/yellow] add this to your shell profile: "
            f"`export PATH=\"{scripts_path}:$PATH\"`"
        )
        console.print(
            "[dim]Apply now in current terminal:[/dim] "
            f"`export PATH=\"{scripts_path}:$PATH\"`"
        )
...
def _run_doctor_check(console, *, dry_run: bool) -> None:
    ...
    if completed.returncode == 0:
        console.print("[green]Doctor check completed.[/green]")
    else:
        console.print("[yellow]Doctor check reported issues.[/yellow]")
    if completed.stdout:
...
def _run_smoke_report(console, *, dry_run: bool) -> None:
    if dry_run:
        console.print(
            "[yellow]Dry run:[/yellow] would run `gittan report --last-week --include-uncategorized --output-format json --quiet`."
        )
        return
    entry = REPO_ROOT / "timelog_extract.py"
    ...
    if isinstance(payload, dict) and payload.get("schema") == "timelog_extract.truth_payload":
        ...
        event_count = totals.get("event_count", "n/a")
        days_with_activity = totals.get("days_with_activity", "n/a")
        hours_estimated = totals.get("hours_estimated", "n/a")
        console.print("[green]Smoke report passed.[/green]")
        console.print(
            f"[dim]schema={payload.get('schema')} | events={event_count} | days={days_with_activity} | hours={hours_estimated}[/dim]"
        )
    else:
...
def setup_global_timelog(
    ...
    console = Console()
    _setup_global_timelog_impl(console, yes=yes, dry_run=dry_run)
...
def setup_wizard(
    ...
    should_setup_timelog = yes or questionary.confirm(
        "Configure global timelog automation now?",
        default=True,
    ).ask()
    ...
    summary_rows.append(
        (
            ...
            "PASS" if not dry_run else "PASS (dry-run)",
            "Checked existing config and offered minimal bootstrap.",
        )
    )
    ...
    summary_rows.append(
        (
            "Doctor check",
            "PASS" if not dry_run else "PASS (dry-run)",
            "Ran (or previewed) source/permission diagnostics.",
        )
    )
        ...
        if should_smoke:
            ...
            summary_rows.append(
                (
                    "Smoke report",
                    "PASS" if not dry_run else "PASS (dry-run)",
                    "Ran (or previewed) JSON smoke report command.",
                )
            )
        else:
