...
# ---------------------------------------------------------------------------
# query_chrome — exception / error paths
# ---------------------------------------------------------------------------


class QueryChromeErrorPathTests(unittest.TestCase):
    """Tests for query_chrome() error handling (the except branch)."""
    ...
    def test_corrupt_db_returns_empty_list(self):
        """A corrupt (non-SQLite) file triggers the except branch and returns []."""
        corrupt = Path(self.tmpdir.name) / "History"
        ...
        result = query_chrome(corrupt, "1=1", cu_from, cu_to)
        self.assertEqual(result, [])

    def test_corrupt_db_temp_file_still_cleaned_up(self):
        """The temp file is removed even when the DB copy is corrupt."""
        corrupt = Path(self.tmpdir.name) / "History"
        ...
        before = set(os.listdir(tempfile.gettempdir()))
        query_chrome(corrupt, "1=1", cu_from, cu_to)
        after = set(os.listdir(tempfile.gettempdir()))
        new_files = [f for f in (after - before) if f.endswith(".db")]
        self.assertEqual(new_files, [], "Temp .db files were not cleaned up after corrupt DB")

    def test_result_rows_ordered_by_visit_time(self):
        """query_chrome returns rows in ascending visit_time order."""
        db_path = Path(self.tmpdir.name) / "History"
        _make_chrome_db(db_path)

        ts_early = datetime(2026, 4, 10, 10, 0, tzinfo=timezone.utc)
        ts_late = datetime(2026, 4, 10, 12, 0, tzinfo=timezone.utc)
        # Insert in reverse order to confirm ORDER BY is applied by SQL, not insertion order.
        _insert_visit(db_path, "https://second.com/", "Second", ts_late)
        _insert_visit(db_path, "https://first.com/", "First", ts_early)

        dt_from = datetime(2026, 4, 10, 9, 0, tzinfo=timezone.utc)
        dt_to = datetime(2026, 4, 10, 13, 0, tzinfo=timezone.utc)
        cu_from, cu_to = self._time_range(dt_from, dt_to)

        rows = query_chrome(db_path, "1=1", cu_from, cu_to)
        self.assertEqual(len(rows), 2)
        self.assertIn("first.com", rows[0][1])
        self.assertIn("second.com", rows[1][1])


# ---------------------------------------------------------------------------
# collect_claude_ai_urls — multi-URL and project-assignment edge cases
# ---------------------------------------------------------------------------


class CollectClaudeAiUrlsMultiTests(unittest.TestCase):
    """Additional tests for collect_claude_ai_urls: multi-URL clauses and project mapping."""
    ...
    def test_multiple_tracked_urls_each_match_independently(self):
        """Two profiles each contribute a claude.ai URL; both visits are collected."""
        ts = datetime(2026, 4, 10, 14, 0, tzinfo=timezone.utc)
        _insert_visit(self.db_path, "https://claude.ai/chat/aaaa", "Chat A", ts)
        _insert_visit(self.db_path, "https://claude.ai/chat/bbbb", "Chat B", ts)

        profiles = [
            {"name": "ProjA", "tracked_urls": ["claude.ai/chat/aaaa"]},
            {"name": "ProjB", "tracked_urls": ["claude.ai/chat/bbbb"]},
        ]
        results = self._call(profiles)
        self.assertEqual(len(results), 2)
    ...
    def test_event_has_correct_source_field(self):
        """Events produced by collect_claude_ai_urls always have source='Claude.ai (web)'."""
        ts = datetime(2026, 4, 10, 14, 0, tzinfo=timezone.utc)
        _insert_visit(self.db_path, "https://claude.ai/chat/xyz", "Chat", ts)
        profiles = [{"name": "P", "tracked_urls": ["claude.ai"]}]
        results = self._call(profiles)
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0]["source"], "Claude.ai (web)")

    def test_backslash_in_tracked_url_does_not_error(self):
        """A tracked_url containing a backslash is escaped and does not raise."""
        ts = datetime(2026, 4, 10, 14, 0, tzinfo=timezone.utc)
        _insert_visit(self.db_path, "https://claude.ai/chat/normal", "Chat", ts)
        # Backslash in tracked URL — _like_escape must double it before passing to SQLite
        profiles = [{"name": "P", "tracked_urls": ["claude.ai\\weirdpath"]}]
        results = self._call(profiles)
        self.assertIsInstance(results, list)

    def test_empty_profiles_list_returns_empty(self):
        """An empty profiles list produces no results and no error."""
        results = self._call([])
        self.assertEqual(results, [])


# ---------------------------------------------------------------------------
# collect_gemini_web_urls — project longest-match assignment
# ---------------------------------------------------------------------------


class CollectGeminiLongestMatchTests(unittest.TestCase):
    """Test the longest-match project selection in collect_gemini_web_urls."""
    ...
    def test_longer_tracked_url_wins_over_shorter(self):
        """When two tracked URLs both match, the longer one determines the project."""
        ts = datetime(2026, 4, 10, 14, 0, tzinfo=timezone.utc)
        _insert_visit(
            self.db_path,
            "https://gemini.google.com/app/specific_chat",
            "Gemini",
            ts,
        )
        # "gemini.google.com" (shorter) matches but "gemini.google.com/app/specific_chat" (longer) also matches.
        profiles = [
            {"name": "General", "tracked_urls": ["gemini.google.com"]},
            {"name": "Specific", "tracked_urls": ["gemini.google.com/app/specific_chat"]},
        ]
        results = self._call(profiles)
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0]["project"], "Specific")

    def test_event_source_is_gemini_web(self):
        """Events produced by collect_gemini_web_urls always have source='Gemini (web)'."""
        ts = datetime(2026, 4, 10, 14, 0, tzinfo=timezone.utc)
        _insert_visit(self.db_path, "https://gemini.google.com/app/abc", "Gemini", ts)
        profiles = [{"name": "P", "tracked_urls": ["gemini.google.com"]}]
        results = self._call(profiles)
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0]["source"], "Gemini (web)")

    def test_backslash_in_tracked_url_does_not_error(self):
        """Backslash in a tracked_url is escaped by _like_escape and does not raise."""
        ts = datetime(2026, 4, 10, 14, 0, tzinfo=timezone.utc)
        _insert_visit(self.db_path, "https://gemini.google.com/app/normal", "G", ts)
        profiles = [{"name": "P", "tracked_urls": ["gemini.google.com\\oddpath"]}]
        results = self._call(profiles)
        self.assertIsInstance(results, list)


# ---------------------------------------------------------------------------
# collect_chrome — null title and profile name as keyword
# ---------------------------------------------------------------------------


class CollectChromeEdgeCaseTests(unittest.TestCase):
    """Edge-case tests for collect_chrome."""
    ...
    def test_null_title_uses_url_as_detail(self):
        """When a page title is NULL, the URL is used as the event detail."""
        ts = datetime(2026, 4, 10, 14, 0, tzinfo=timezone.utc)
        # Insert a visit with no title (NULL stored as empty string via our helper).
        # Insert directly to control NULL vs empty:
        visit_time_cu = int(ts.timestamp() * 1_000_000) + EPOCH_DELTA_US
        conn = sqlite3.connect(str(self.db_path))
        cur = conn.cursor()
        cur.execute("INSERT INTO urls (url, title) VALUES (?, NULL)", ("https://myproject.io/page",))
        url_id = cur.lastrowid
        cur.execute("INSERT INTO visits (url, visit_time) VALUES (?, ?)", (url_id, visit_time_cu))
        conn.commit()
        conn.close()
        ...
        results = self._call(profiles)
        self.assertEqual(len(results), 1)
        # When title is None/NULL, detail should be the URL (truncated to 70 chars)
        self.assertIn("myproject.io", results[0]["detail"])

    def test_profile_name_used_as_keyword(self):
        """collect_chrome includes the profile name in all_keywords, not just match_terms."""
        ts = datetime(2026, 4, 10, 14, 0, tzinfo=timezone.utc)
        # URL contains the profile name "acmecorp" but no match_terms
        _insert_visit(self.db_path, "https://acmecorp.io/dashboard", "Dashboard", ts)

        profiles = [{"name": "acmecorp", "match_terms": []}]
        results = self._call(profiles)
        # The profile name "acmecorp" should appear in all_keywords and match the URL
        self.assertEqual(len(results), 1)

    def test_keywords_are_lowercased_before_matching(self):
        """all_keywords are lowercased; LOWER(u.url) LIKE is case-insensitive."""
        ts = datetime(2026, 4, 10, 14, 0, tzinfo=timezone.utc)
        _insert_visit(self.db_path, "https://MyProject.IO/page", "Page", ts)

        profiles = [{"name": "Proj", "match_terms": ["MYPROJECT"]}]
        results = self._call(profiles)
        self.assertEqual(len(results), 1)

    def test_profile_name_with_special_chars_does_not_error(self):
        """A profile name containing % or _ in the name field does not cause SQL errors."""
        ts = datetime(2026, 4, 10, 14, 0, tzinfo=timezone.utc)
        _insert_visit(self.db_path, "https://site.io/page", "Page", ts)

        # Profile name with both SQL LIKE metacharacters
        profiles = [{"name": "proj_50%done", "match_terms": []}]
        results = self._call(profiles)
        self.assertIsInstance(results, list)

    def test_empty_db_returns_empty_list(self):
        """collect_chrome returns [] when history DB exists but contains no visits."""
        profiles = [{"name": "Proj", "match_terms": ["anything"]}]
        results = self._call(profiles)
        self.assertEqual(results, [])
