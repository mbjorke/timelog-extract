...
## 2026-07-27 - Converting Manual Typing Prompts to Interactive Select Menus for Better CLI UX
**Learning:** Replaced manual typing prompts (e.g., in `gittan intent`) with highly polished `questionary.select` lists containing the known/configured options, along with explicit "Skip" and "Cancel" escape hatches. This completely bypasses error-prone typing loop validation and maintains perfect interactive alignment with other wizard paths in the Gittan CLI.
**Action:** Refactored the interactive selection loop in `core/cli_intent.py` to use `questionary.select`. Handled cancellation (None/Cancel choice) gracefully with a `CLR_VALUE_ORANGE` aborted message and exit code `130`. Added comprehensive interactive prompt unit tests in `tests/test_intent_store.py`.
