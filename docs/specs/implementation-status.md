...
| `docs/specs/working-directory-anchor-signal.md`        | `built`        | Claude Code CLI events preserve `context_dir` (cwd leaf); `projects-audit` emits `top_dirs` + `--write-anchor-plan`; `projects-anchor` applies match_term additions (dry-run + backup). Auto-surfaced in `status` (one-line warning + interactive map, `--no-anchor-nudge`) and `report` (multi-line nudge). Tests: `test_collectors_claude_code_context_dir.py`, `test_projects_audit.py`, `test_report_nudges.py`, `test_anchor_nudge.py`. Follow-up: Cursor/Gemini `context_dir`, React Ink overlay. |
