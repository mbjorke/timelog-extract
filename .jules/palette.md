...
## 2026-07-23 - Standardizing `evidence` CLI Outputs, Error Messaging and Chain Integrity
**Learning:** Terminal errors, warnings, and states must not use hardcoded `[red]` or arbitrary unstyled labels. Incorporating the standard `FAIL_ICON`, semantic theme accent (`CLR_VALUE_ORANGE`), and a clean/reusable 'Next:' follow-up message when errors occur maintains visual cohesion, trust, and usability. Additionally, testing CLI subcommands using `CliRunner` requires importing the Typer app from `core.cli` (instead of `core.cli_app`) to guarantee that all subcommands are correctly registered before invocation.
**Action:** Redesigned the output of `gittan evidence` for mutually exclusive options, validation errors, and broken integrity chains in `core/cli_evidence.py` to use `FAIL_ICON`, `CLR_VALUE_ORANGE`, and proper closing tags `[/]`. Added a comprehensive test suite `CliEvidenceTests` in `tests/test_cli_evidence.py` to verify formatting, error messages, and subcommands.
