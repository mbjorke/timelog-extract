...
4. Prefer non-destructive config handling:
   ...
   - for **gap / domain triage automation**, use `gittan triage --json` (read-only plan) before `--yes`; contract in **`docs/runbooks/gittan-triage-agents.md`**. Cursor **`/gittan-triage-review`** references the same runbook via **`.cursor/commands/gittan-triage-review.md`**.
