...
# Ensure the repository root is in python path so core can be imported stand-alone
...
def http_json(method, url, body=None, headers=None, timeout=20):
    if urlparse(url).scheme.lower() != "https":
        raise ValueError(
            "Briox API base URL must use HTTPS to prevent token leakage over unencrypted HTTP"
        )
