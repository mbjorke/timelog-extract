...
- **Merge gate (mandatory, even for SAFE):** immediately before every `gh pr merge`, run `scripts/rabbit_loop.sh --merge-gate [--pr N]`. `MERGE_GATE: BLOCKED` (unresolved review threads — bot or human — or threads could not be verified) → do **not** merge; reply + resolve every thread per the review close-out routine, then re-run until `CLEAR`.
