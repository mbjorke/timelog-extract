...
**Role**  
You work in the **timelog-extract** repository (CLI entry point `gittan`, product name Gittan). Read `README.md`, `docs/V1_SCOPE.md`, and `docs/VERSIONING.md`. Inspect existing setup and diagnostics code (`core/global_timelog_setup_lib.py`, `core/cli_global_timelog_setup.py`, `core/cli_doctor_sources_projects.py`). Respect **local-first** behavior: do not add mandatory cloud upload paths for user data.
