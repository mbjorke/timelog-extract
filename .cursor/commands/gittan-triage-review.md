...
2. Confirm **`gittan review --json`** stays **read-only** (no config writes). Legacy **`gittan triage-map --json`** and **`gittan triage --json`** must also remain read-only until removed.
