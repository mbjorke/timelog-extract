...
Timely Memory is **opt-in and off by default** (`--timely-memory-source on`). If the Timely Memory desktop tracker is installed, it keeps a local buffer of foreground samples on disk; Gittan can read that buffer **read-only** (WAL-safe copy, the third-party file is never written) as a **presence comparator** (`coverage_comparator` role, same class as Screen Time). Privacy: only sample **timestamps** are read — window titles, app names, and URLs in the buffer are never accessed, and nothing leaves the machine. The result is per-day presence hours in `collector_status` for coverage/gap comparison; it never creates classified project time and never affects billable totals. When the buffer is missing (app not installed or purged), the source reports "not found" and the report completes normally. Spec: [`../task-prompts/timely-memory-collector-spike-task.md`](../task-prompts/timely-memory-collector-spike-task.md).
