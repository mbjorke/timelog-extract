...
- CLI: `gittan review` filters Cursor/IDE tooling noise (skills-cursor sync, `wt-*` worktrees, File Watcher/FSEvents, opened-repository churn) from uncategorized clusters; still prefer `triage-map` for URL mapping.
- CLI: **`gittan review`** is now the canonical URL → project mapping flow (same as former `triage-map`; `--json` for read-only candidates). Legacy log-cluster loop: **`gittan review --uncategorized`**. **`gittan triage-map`** remains as a deprecated alias. Report runs no longer show inline **Mapping suggestions** prompts. Other deprecated commands: **`triage*`**, **`suggest-rules`**, **`apply-suggestions`**.
...
- CLI / UX: realign terminal palette and command heroes with the public Gittan brand; `status`, `doctor`, `report`, and source summaries now use evidence/review/approval language and shared honey/blueberry/success tokens.
- Cast player pipeline: add `gittan cast` and `CastWriter` for asciicast v3 files with Gittan semantic `"g"` events plus plain terminal fallback output.
- Cast quality: use presentation timing by default, include fallback text tables for terminal playback, and write UTF-8 JSON lines without escaping arrows or Nordic characters.
- Tests: cover cast writer timing, fallback table output, UTF-8 serialization, and command-level `.cast` generation.
...
- Live terminal demo: deterministic mock output now supports a stable demo flow (`gittan doctor`, `gittan report --today --source-summary`, and `gittan report --today --format json`) with Truth Standard language.
