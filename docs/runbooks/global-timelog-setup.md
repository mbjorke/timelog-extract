...
- Default behavior is automatic: the CLI uses `~/.gittan/timelog_projects.json` when present, otherwise `~/.gittan-<user>/timelog_projects.json`. Cwd-local `timelog_projects.json` is not auto-selected (see `gittan config path`).
