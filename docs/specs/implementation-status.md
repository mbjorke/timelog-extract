...
| `docs/specs/local-evidence-shadow-log.md`              | `not started`  | Draft spec for opt-in local retained evidence when upstream logs rotate or disappear. Measure-first slice-1 backlog: `docs/task-prompts/local-evidence-shadow-log-slice1-task.md` (record contract + volume spike before engine choice; JSONL-first vs tiered Parquet/DuckDB gated on measured rate; pg_duckdb ruled out). |
