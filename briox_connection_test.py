...
class RejectHttpRedirectHandler(urlrequest.HTTPRedirectHandler):
    ...
    def redirect_request(self, req, fp, code, msg, headers, newurl):
        if (urlparse(newurl).scheme or "").lower() == "http":
            raise urlerror.URLError("Briox redirect to insecure http:// rejected to protect credentials")
...
def http_json(method, url, body=None, headers=None, timeout=20):
    if not url.lower().startswith("https://"):
        raise ValueError("Briox API base URL must use HTTPS to prevent credential leakage over unencrypted HTTP")
