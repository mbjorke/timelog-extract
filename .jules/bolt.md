...
**Action:** Always prefer `datetime.fromisoformat` over `datetime.strptime` for standard ISO-8601 date and time parsing across core utilities and collectors alike. **Caveat added 2026-07-26 — read `docs/specs/timestamp-standard.md` §4 before applying this:** on the declared 3.10 floor `fromisoformat` rejects a `Z` suffix and rejects compact/basic formats like `20260709T162324`, both of which 3.11+ accepts. Strip `Z` to `+00:00` first; use manual int slicing for compact formats. Applying this action literally to those inputs passes locally on 3.11+ and breaks the 3.10 CI job.
**Numbers re-measured 2026-07-26 — the ratios above do not reproduce.** These entries recorded speedups with no script attached, so nobody could re-run them. `scripts/bench_timestamp_parsing.py` now does, and on CPython 3.11.15 it measures 25.4x here (not 32.7–37.1x), 25.5x for the #401 log-line case (not 4.4–6.0x), and 7.1x for the #456 compact-folder case (not 11.6x). The direction holds everywhere; the magnitudes are machine- and interpreter-dependent. Also unsupported: "decreases hotpath report compilation times noticeably" — `get_date_range` is called twice per report, so that conversion saved roughly 8 microseconds. **Attach a re-runnable script to any future performance claim in this journal**; a ratio in prose cannot be verified or refuted later.
...
**Learning:** Initializing urllib HTTPS openers with `build_https_opener` or `HTTPSHandler` at the module/import level in collectors (like `github.py`, `toggl.py`, `jira.py`) triggers `ssl.create_default_context()` and `set_default_verify_paths()` synchronously during Python's import phase. This scans the host OS for CA certificates, incurring disk I/O on every CLI invocation even when no network collector is used. Lazy-loading these openers inside `urlopen` delays that until a request is actually made.
**Action:** Lazy-initialize network openers, sockets and certificate managers inside functions rather than at module level. Pin the contract with tests: deferral is only a win if import really builds nothing, and only *safe* if first use still yields the same hardened opener — the redirect and plain-HTTP guards are exactly what a careless deferral drops silently.
