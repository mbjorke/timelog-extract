...
| `docs/specs/local-evidence-shadow-log.md`              | `partial`      | Slice 1 (record contract + volume spike, GH-151/#153), item 3 (durable opt-in `--shadow-log on` capture to `~/.gittan/evidence/`, append-only JSONL + per-month hash chain), and item 5 (`gittan evidence` health surface) are built; replay for closed windows (item 4) remains. Backlog: `docs/task-prompts/local-evidence-shadow-log-slice1-task.md`. Engine decided JSONL-first from measured volume; pg_duckdb ruled out. |
